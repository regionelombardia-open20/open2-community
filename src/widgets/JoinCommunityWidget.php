        ...
        if (!empty($urlRedirect)) {
            ...
            if (is_null($userCommunity)) {
                ...
                if ($userCommunity->status == CommunityUserMm::STATUS_WAITING_OK_COMMUNITY_MANAGER) {
                    ...
                    $hasJoinButton = true;
                    $icon = 'sign-in';
                    $title = AmosCommunity::t('amoscommunity', '#accedi') . AmosIcons::show($icon);
                    $titleLink = AmosCommunity::t('amoscommunity', 'Sign in');
                    $buttonUrl = '/community/join?id=' . $model->id;
                    //$this->btnOptions['class'] .= ' ' . self::btnJoinSelector() . ' ';
                    $this->btnOptions['data-community_id'] = $model->id;
                    $this->btnOptions['class'] .= ' ' . 'linkToCommunityJquery';
                }
            }
        }

        $buttonAdminJoin = '';
        if (!$hasJoinButton && \Yii::$app->user->can('AMMINISTRATORE_COMMUNITY')) {
            $buttonAdminJoin = Html::a(AmosIcons::show('sign-in'), '/community/join?id=' . $model->id, [
                'data-community_id' => $model->id,
                'class' => $this->btnOptions['class'] .= ' ' . 'linkToCommunityJquery',
                'title' => AmosCommunity::t('amoscommunity', '#accedi')
            ]);
        }
